  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${BACKEND_URL}/names/generate-names`,
        requestData,
        {
      ...
      <div className="bg-white shadow-lg p-6 rounded-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <select
            ...
            className="w-full px-4 py-2 border border-pink-300 rounded-md focus:ring-2 focus:ring-pink-500"
          >
        ...
        <div className="bg-white shadow-lg p-6 rounded-xl mt-6 w-full max-w-md">
          ...
          <ul className="mt-4 space-y-2">
            {suggestedNames.map((nameObj, index) => (
              <li
                key={index}
                className="text-center bg-pink-200 py-2 px-4 rounded-md"
              >
                <strong>{nameObj.name}</strong> - {nameObj.meaning} (
                {nameObj.origin})
              </li>
            ))}
          </ul>
